 ...
 */
public class MyASARCalibrator extends MyBaseCalibrator implements MyCalibrator {
...
//    private double halfLightSpeedByRefSlantRange = Constants.halfLightSpeed / refSlantRange;
     ...
     * @param mustPerformRetroCalibration If true, retro-calibration must be
     * performed if it is applicable.
    ...
    /**
     * Get antenna elevation correction flag and range spreading compensation
     * flag from Metadata.
     *
    ...
    /**
     * Get calibration factors from Metadata for each band in the product. Here
     * it is assumed that the calibration factor values do not change in case
     * that there are multiple records in the Main Processing Parameters data
     * set.
     */
    ...
    /**
     * Get reference elevation angle and antenna pattern gain from auxiliary
     * file for single swath product.
     *
     * @param fileName The auxiliary data file name
     * @param swath The swath name.
     * @param pol The polarizations for 2 bands.
     * @param numOfGains The number of gains for given swath and polarization
     * (201).
    ...
    /**
     * Set power coefficient used in range spreading loss compensation
     * computation for slant range images.
     */
    ...
    public void computeTile(Band targetBand, Tile targetTile) throws OperatorException {
        ...
        for (int y = y0, yy = 0; y < maxY; ++y, ++yy) {
            ...
            incidenceTPGInterp.getPixels(x0, y, w, 1, incidenceAnglesArray, new NullProgressMonitor(), TiePointInterpolator.InterpMode.QUADRATIC);

            if (applyRangeSpreadingCorr) {
                slantRangeTPGInterp.getPixels(x0, y, w, 1, slantRangeTimeArray, new NullProgressMonitor(), TiePointInterpolator.InterpMode.QUADRATIC);
            }
    ...
    /**
     * Compute antenna pattern for the middle row of the given tile for single
     * swath product. Here it is assumed that the elevation angles for pixels in
    ...
    /**
     * Compute antenna pattern for the middle row of the given tile for wide
     * swath product. Here it is assumed that the elevation angles for pixels in
     * the same column are the same.
     *
     * @param x0 The x coordinate of the upper left point in the current tile.
    ...
    /**
     * Compute antenna pattern gains for the given elevation angle using linear
     * interpolation.
     *
     ...
     * @param satelliteHeight The distance from satelitte to the Earth centre
     * (in meters).
     * @param sceneToEarthCentre The distance from the backscatter element to
     * the Earth centre (in meters).
    ...
    /**
     * Remove the antenna pattern compensation and range spreading loss applied
     * to the pixel.
     *
     * @param x The x coordinate of the pixel in the source image.
     * @param y The y coordinate of the pixel in the source image.
     ...
     * @param subSwathIndex The sub swath index for current pixel for wide swath
     * product case.
     * @return The pixel value with antenna pattern compensation and range
     * spreading loss correction removed.
     */
     ...
     * @param refElevationAngle The reference elevation angles for different
     * swathes or sub swathes.
     * @param antennaPattern The antenna pattern array. For single swath
     * product, it contains two 201-length arrays corresponding to the two bands
     * of different polarizations. For wide swath product, it contains five
     * 201-length arrays with each for a sub swath.
     * @param compSubSwathIdx The boolean flag indicating if sub swath index
     * should be computed.
     * @param subSwathIndex The sub swath index for current pixel for wide swath
     * product case.
    ...
    /**
     * Apply calibrations to the given point. The following calibrations are
     * included: calibration constant, antenna pattern compensation, range
     * spreading loss correction and incidence angle correction.
     *
     * @param v The pixel value.
     * @param slantRange The slant range (in m).
     * @param satelliteHeight The distance from satellite to earth centre (in
     * m).
     * @param sceneToEarthCentre The distance from the backscattering element
     * position to earth centre (in m).
